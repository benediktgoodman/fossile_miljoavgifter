...
class DataImporter():
    ...
    def add_years(self, year_as_datetime=True):
...
Hint:
    ...
    @staticmethod
    def __year_dtype_fixer(storage_dict):
        """
        Given a dictionary of pandas dataframes, this function converts the 'aar' column of 
        dataframes that have this column to a datetime dtype and extracts the year, then
        ...
        -------
        dict
            A dictionary of pandas dataframes, where the 'aar' column of dataframes that have
            this column is converted to a datetime dtype and the year is extracted.
        """
        # If aar is in columns of dataframes, turns it into year format
        if isinstance(storage_dict, dict):
            storage_dict = {key: df.assign(aar=pd.to_datetime(df['aar'], format='%Y').dt.year)
                            if 'aar' in df.columns else df
                            for key, df in storage_dict.items()}
    ...
    def year_duplicate_tidy_func(self,
        ...
        for df in storage_dict.keys():
            ...
            # Renames dupl_col2
                ...
                storage_dict[df] = (storage_dict[df]
                                    .rename(columns={dupl_col2 : dupl_col1})
                                    #.assign(dupl_col1 = pd.to_datetime(dupl_col1))
                                   )

            # Drops dupl_col2 if dupl_col1 is present
            else:
                storage_dict[df] = (storage_dict[df]
                                    .drop(dupl_col2, axis=1)
                                    #.assign(dupl_col1 = pd.to_datetime(dupl_col1))
                                   )
        # Fixes
        self.sorted_dataframes = self.__year_dtype_fixer(storage_dict)
