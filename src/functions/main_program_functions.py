...
def multi_subsetter(data: dict,
    ...
    """
    # Make mask formatted as query syntax
...
def associate_codes(df_in: 'pd.DataFrame',
    ...
    # Merge the two dataframes to create a mapping between the two code sets
    mapping = pd.merge(df[[ind_code_col]], coding_subset, on=ind_code_col, how='left')

    # Associate product-code in output dataframe  w/ nr-code & add as new column
    d = dict(zip(mapping[ind_code_col], mapping[nr_code_col]))
...
def rounding_error_dealer(df_in,
    ...
    factor = np.divide(df[idx]['diff'], df[idx]['n_max_obs'])
    corrected_fees = df[idx][estimated_fee_col] + factor
